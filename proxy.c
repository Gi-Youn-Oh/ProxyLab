...
// 함수 전방 선언
void doit(int connfd); // client(request)->proxy(read&write)->server(read&write)->proxy(read&write)->client
void clienterror(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg); // client request config
void parse_uri(char *uri,char *hostname,char *path,int *port); // uri에서 요청 해석,분할,검증
int make_request(rio_t* client_rio, char *hostname, char *path, int port, char *hdr, char *method); //client 요청을 서버로 보낼 준비
