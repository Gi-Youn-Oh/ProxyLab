...
void doit(int fd)  // HTTP 트랜잭션 수행
{
    ...
    printf("%s %s %s\n", method, uri, version);
    if(strcasecmp(method, "GET") && strcasecmp(method, "HEAD")){ // 만일 클라이언트가 GET 이 아닌 다른 메소드를 요청하면 에러메시지를 보내고 main루틴으로 돌아온다.
    ...
    if (is_static) { /* Serve static content */  // 요청이 정적컨텐츠를 위한 것이라면
        ...
        serve_static(fd, filename, sbuf.st_size, method); // 보통파일이고 읽기권한이 있다면 클아이언트에게 제공
    }
        ...
        serve_dynamic(fd, filename, cgiargs, method); // 동적컨텐츠이고 실행가능하면 제공
    }
}
